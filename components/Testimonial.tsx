...
const Testimonial: React.FC = () => {
  ...
  return (
    <section className="bg-white py-24 md:py-36 border-b border-[#8d5527]/5 relative overflow-hidden leaf-pattern group">
      {/* Decorative Elements */}
      ...
      <button 
        ...
        className="absolute left-4 md:left-10 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/80 backdrop-blur-md border border-emerald-100 flex items-center justify-center text-[#8d5527] opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-[#8d5527] hover:text-white shadow-lg"
      >
      ...
      <button 
        ...
        className="absolute right-4 md:right-10 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/80 backdrop-blur-md border border-emerald-100 flex items-center justify-center text-[#8d5527] opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-[#8d5527] hover:text-white shadow-lg"
      >
      ...
      <div className="container mx-auto px-6 max-w-5xl relative z-10">
        <div className="relative">
          ...
          <div className="bg-white/40 backdrop-blur-sm border border-emerald-100/50 p-8 md:p-16 rounded-[2rem] md:rounded-[4rem] shadow-[0_30px_100px_-20px_rgba(6,78,59,0.08)] relative overflow-hidden group/card hover:shadow-[0_40px_120px_-20px_rgba(6,78,59,0.12)] transition-all duration-700 min-h-[400px] flex flex-col justify-center">
            {/* Inner Glow Effect */}
            <div className="absolute -top-24 -right-24 w-48 h-48 bg-[#efe2d2] rounded-full blur-3xl group-hover/card:bg-[#efe2d2]/50 transition-colors duration-700"></div>
            
            <div className="relative z-10 animate-fade-in">
              <p className="text-xl sm:text-2xl md:text-3xl font-serif text-[#8d5527] italic leading-relaxed mb-12 text-center md:text-left transition-all duration-500">
                "{current.text}"
              </p>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-8 border-t border-emerald-100/80 pt-10">
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex flex-col">
                    <span className="text-base md:text-lg font-black tracking-widest text-[#8d5527] uppercase">{current.name}</span>
                ...
                <div className="flex items-center gap-8">
                  ...
                  <div className="flex flex-col items-center md:items-end gap-1">
                    <span className="text-[10px] font-bold tracking-[0.2em] text-[#8d5527]/40 uppercase mb-2">Verified on {current.source}</span>
        ...
        <div className="flex justify-center gap-3 mt-12">
          {TESTIMONIALS.map((_, idx) => (
            <button 
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-8 bg-[#bf885e]' : 'bg-emerald-200 hover:bg-emerald-300'}`}
            />
          ))}
        </div>
      </div>
