  ...
  return (
    <section className="py-12 md:py-20 relative overflow-hidden leaf-pattern">
      <div className="container mx-auto px-4 md:px-6 max-w-7xl relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 lg:gap-8 h-[600px] sm:h-[700px] md:h-[600px] lg:h-[700px]">
          {IMAGES.map((src, idx) => {
            ...
            return (
              <div 
                ...
                className={`relative transition-all duration-1000 ease-in-out cursor-pointer overflow-hidden shadow-2xl rounded-sm
                  ${isActive 
                    ? 'w-full md:w-[50%] h-[60%] md:h-full z-20 shadow-emerald-900/20' 
                    : 'w-full md:w-[25%] h-[20%] md:h-[85%] z-10 opacity-80 md:opacity-100'}
                `}
              >
                {/* Image Layer */}
                <img 
                ...
                <div className={`hidden md:flex absolute inset-0 items-center justify-center pointer-events-none transition-opacity duration-500 ${!isActive && hoverIdx === idx ? 'opacity-100' : 'opacity-0'}`}>
                  <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-full border border-white/50 flex items-center justify-center animate-pulse bg-white/10 backdrop-blur-sm">
                    <i className={`fa-solid ${idx < activeIdx ? 'fa-arrow-right' : 'fa-arrow-left'} text-white text-xl lg:text-2xl`}></i>
                  </div>
                </div>
