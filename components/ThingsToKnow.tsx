  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 md:top-6 md:right-6 flex flex-col items-end gap-2 z-20 pointer-events-auto">
        ...
        <span className={`text-[6px] md:text-[7px] font-bold opacity-0 group-hover:opacity-100 tracking-[0.3em] uppercase transition-opacity whitespace-nowrap select-none ${isFocused ? 'text-[#064E3B]/40 group-hover:text-emerald-600' : 'text-emerald-400'}`}>
          Click to Dock
        </span>
      </div>
      ...
      <div className={`flex flex-col items-center justify-center w-full h-full relative z-10 pointer-events-none transition-all duration-700 ${isFocused ? 'scale-[1.02]' : 'scale-100'}`}>
        ...
        <span className={`absolute -right-4 -bottom-4 font-serif font-bold transition-all duration-1000 select-none pointer-events-none transform group-hover:scale-110 ${isFocused ? 'text-[#064E3B]/[0.05] text-[150px] md:text-[250px]' : 'text-white/[0.01] text-[100px] md:text-[150px]'}`}>
          {point.id}
        </span>

        {/* Subtitle */}
        <p className={`font-bold tracking-[0.3em] md:tracking-[0.5em] uppercase mb-1 md:mb-2 transition-colors ${isFocused ? 'text-emerald-800/80 group-hover:text-emerald-500 text-[8px] md:text-[10px]' : 'text-emerald-400/40 group-hover:text-emerald-400 text-[7px] md:text-[8px]'}`}>
          {point.subtitle}
        </p>

        {/* Title */}
        <h3 className={`font-serif mb-2 md:mb-4 transition-colors leading-tight text-center ${isFocused ? 'text-[#064E3B] group-hover:text-emerald-600 text-3xl md:text-5xl' : 'text-white group-hover:text-emerald-300 text-xl md:text-2xl'}`}>
          {point.title}
        </h3>
        
        {/* Description */}
        <p className={`leading-[1.6] md:leading-[2] font-light mb-4 md:mb-6 transition-all duration-500 text-center max-w-xl ${isFocused ? 'text-[#064E3B]/70 group-hover:text-emerald-700 text-sm md:text-lg italic' : 'text-white/60 group-hover:text-white/90 text-[12px] md:text-[13px]'}`}>
          {point.description}
        </p>
        
        <div className="flex items-center gap-3 md:gap-4">
          <div className="w-1 md:w-1.5 h-1 md:h-1.5 rounded-full bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,1)] animate-ping-slow"></div>
          <span className={`font-bold tracking-[0.2em] md:tracking-[0.3em] uppercase transition-colors ${isFocused ? 'text-[#064E3B]/80 group-hover:text-emerald-600 text-[8px] md:text-[10px]' : 'text-white/80 group-hover:text-emerald-400 text-[8px] md:text-[9px]'}`}>
            {point.extra}
          </span>
        </div>
          ...
          <div className="w-full lg:w-[45%] relative pl-0 lg:pl-[140px]">
            <div className="lg:sticky lg:top-40">
              <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
                ...
                <span className="text-[8px] md:text-[10px] font-bold tracking-[0.4em] md:tracking-[0.6em] uppercase text-emerald-400/80">WILDERNESS INTELLIGENCE</span>
              </div>
              ...
              <h2 className="text-4xl sm:text-5xl md:text-8xl font-serif text-white mb-6 md:mb-10 leading-[1.05] tracking-tighter">
                ...
                <span className="italic text-emerald-400">To Know</span>
              </h2>
              
              <div className="space-y-6 md:space-y-10">
                <p className="text-sm md:text-[16px] leading-[1.8] md:leading-[2.2] text-white/70 font-light italic border-l-2 border-emerald-500/30 pl-6 md:pl-10 max-w-md">
                  {allDocked 
                ...
                >
                  <div className={`w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl border border-emerald-500/20 flex items-center justify-center text-emerald-400 text-xl md:text-2xl transition-all duration-700 group-hover/btn:bg-emerald-500 group-hover/btn:text-white group-hover/btn:scale-110 group-active/btn:scale-95 ${allDocked ? 'bg-emerald-500/10' : ''}`}>
                    <i className={`fa-solid ${allDocked ? 'fa-plus' : 'fa-minus'} animate-blink`}></i>
                  </div>
                  <div>
                    <p className="text-[9px] md:text-[10px] font-bold tracking-[0.3em] md:tracking-[0.4em] text-white uppercase mb-1 md:mb-2 group-hover/btn:text-emerald-400 transition-colors">
                      {allDocked ? 'RESTORE GRID' : 'DOCK ALL'}
                    </p>
                    <p className="text-[8px] md:text-[9px] text-emerald-400/60 tracking-wider font-medium uppercase leading-relaxed max-w-[200px] md:max-w-none">
                      {allDocked 
