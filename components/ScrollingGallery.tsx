...
const ScrollingGallery: React.FC = () => {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full overflow-hidden bg-[#fbf7f2] py-16 sm:py-20 md:py-32 border-t border-[#8d5527]/10 leaf-pattern ${isTouchDevice ? "cursor-auto" : "cursor-none"}`}
    ...
    >
      ...
      {!isTouchDevice && (
        ...
        >
          ...
          >
            <div className="text-[#664831] text-[10px] font-bold tracking-[0.6em] uppercase mb-2 drop-shadow-sm">
              {isPaused ? "EXPLORE" : "SWIPE WILD"}
            </div>
            <div className="w-12 h-px bg-emerald-800/20"></div>
            <div className="mt-3 flex gap-2">
              <i
                className={`fa-solid fa-chevron-left text-[8px] text-[#bf885e]/40 ${!isPaused ? "animate-pulse" : ""}`}
              ></i>
              <div
                className={`w-1 h-1 rounded-full bg-[#efe2d2]0 ${isPaused ? "animate-ping" : ""}`}
              ></div>
              <i
                className={`fa-solid fa-chevron-right text-[8px] text-[#bf885e]/40 ${!isPaused ? "animate-pulse" : ""}`}
              ></i>
            </div>
          </div>
      ...
      <div className="flex" ref={scrollRef}>
        {[...IMAGES, ...IMAGES].map((src, idx) => (
          <div
            key={idx}
            className="shrink-0 w-[220px] sm:w-[260px] md:w-[480px] px-2 sm:px-3 md:px-6"
          >
            <div className="aspect-4/5 overflow-hidden rounded-[2.5rem] md:rounded-[4rem] shadow-[0_40px_80px_-20px_rgba(6,78,59,0.12)] group relative border border-emerald-900/5 bg-white transition-all duration-700 hover:shadow-emerald-900/20">
              ...
              <div className="absolute inset-0 bg-linear-to-b from-transparent via-transparent to-emerald-950/40 opacity-60 group-hover:opacity-20 transition-opacity duration-700"></div>
              ...
              <div className="absolute top-6 left-6 md:top-10 md:left-10 opacity-0 md:group-hover:opacity-100 transition-all duration-500 translate-y-4 md:group-hover:translate-y-0">
                <span className="text-white font-serif italic text-2xl md:text-4xl opacity-40">
